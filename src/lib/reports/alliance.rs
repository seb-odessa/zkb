...
impl Alliance {
    ...
    fn perform_report(arg: &String, ctx: &Context, report_type: ReportType) -> String {
        ...
        } else if let Some(ref id) = find_id("alliance", arg, ctx) {
            Self::report_by_id(id, ctx, report_type)
        } else {
            format!("<div>alliance {} was not found</div>", arg)
        }
    }
    ...
    fn report_by_id(id: &i32, ctx: &Context, report_type: ReportType) -> String {
        ...
        lazy(&mut output, format!("history/alliance_wins/{}/{}", id, 60), &ctx);
        reports::div(&mut output, "Losses");
        lazy(&mut output, format!("history/alliance_losses/{}/{}", id, 60), &ctx);
